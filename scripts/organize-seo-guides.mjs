...
const cards = (items) => items.map((guide) => {
  ...
  const href = guide.slug === "preparare-fototessere-documenti" ? `${guide.slug}/index.html?v=0.1.0` : `${guide.slug}/index.html`;
  return `<a class="guide-card" href="${href}"><span class="guide-product">Manuale ${esc(guide.tool)}${availability}</span><h3>${esc(guide.title)}</h3><p>${esc(guide.problem)}</p><span class="guide-link">Apri il manuale →</span></a>`;
}).join("");
