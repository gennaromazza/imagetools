...
export type ImageConverterPresetId =
  | "web-quality"
  | "web-light"
  | "social"
  | "quick-preview"
...
export type NetworkDriveHealth =
  | "unknown"
  | "healthy"
  | "missing-mapping"
  | "ghost-mapping"
...
export type ArchivioArchiveRenamePhase =
  | "idle"
  | "preparing"
  | "renaming"
  | "counting"
...
export interface FileXDesktopApi {
  ...
  findNestedPhotoSelectorProjects: (
    rootPath: string,
    options?: DesktopNestedProjectScanOptions,
  ) => Promise<DesktopPhotoSelectorProjectLocation[]>;
  ...
  ejectArchivioSdCard: (sdPath: string) => Promise<{ ok: boolean; message: string }>;
  showArchivioFlowWindow: () => Promise<{ ok: true }>;
  ...
  cancelImageConverterJob: () => Promise<{ ok: boolean; active: boolean }>;
  openImageConverterFolder: (folderPath: string) => Promise<{ ok: boolean }>;
  chooseImageFileFinderSourceFolder: () => Promise<string | null>;
  chooseImageFileFinderDestinationFolder: () => Promise<string | null>;
  validateImageFileFinderFolder: (folderPath: string) => Promise<{ ok: boolean; path?: string; error?: string }>;
