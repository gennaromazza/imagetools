export type DesktopToolId = "suite-launcher" | "image-party-frame" | "batch-print-layout" | "id-photo" | "archivio-flow" | "image-converter" | "image-file-finder" | "cache-sweep" | "filex-send" | "backup-guard" | "photo-selector-app" | "album-flow";
...
export interface DesktopAlbumFlowHandoffManifestV1 {
    ...
    createdAt: string;
    expiresAt: string;
    labels: Array<{ id: string; name: string; color?: string; source: "selector-custom" | "selector-color" | "album" }>;
    assets: DesktopAlbumFlowHandoffAsset[];
}
export interface DesktopAlbumFlowHandoffRequest { manifest: DesktopAlbumFlowHandoffManifestV1; }
export type DesktopPhotoToolHandoffTargetToolId = "image-party-frame" | "batch-print-layout" | "id-photo" | "album-flow";
...
export type DesktopToolInstallStatus = "installed" | "not-installed" | "update-available" | "suite-update-required";
...
export interface DesktopToolUpdateCheckResult {
    ...
    reason?: "up-to-date" | "new-version" | "not-installed" | "not-found" | "suite-update-required";
}
...
export interface FileXDesktopApi {
  sendAlbumFlowHandoff: (request: DesktopAlbumFlowHandoffRequest) => Promise<DesktopPhotoToolHandoffSendResult>;
    ...
    sendPhotoSelectionToTool: (request: DesktopPhotoToolHandoffRequest) => Promise<DesktopPhotoToolHandoffSendResult>;
    consumePhotoSelectionHandoff: (projectPath: string) => Promise<DesktopPhotoToolHandoff | null>;
  ...
  getLicenseState: (refresh?: boolean) => Promise<DesktopLicenseState>;
    ...
    finalizeAtomicWriteTransaction: (transactionId: string, recovery?: DesktopAtomicWriteFinalizeRecovery) => Promise<boolean>;
    ...
    notifyBackupGuardProject: (notification: BackupGuardProjectNotification) => Promise<{
        ok: boolean;
    }>;
    ...
    listArchivioJobSubfolders: (jobId: string, author?: string) => Promise<{
        subfolders: string[];
    }>;
