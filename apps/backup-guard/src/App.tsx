...
const labels = { "copy-to-clone": "Da aggiungere al clone", "import-from-clone": "Nuovi sul clone", "delete-from-clone": "Da eliminare dal clone", "restore-to-clone": "Da ripristinare sul clone", "rename-on-clone": "Da rinominare sul clone", conflict: "Conflitti" } as const;
...
export default function App() {
  ...
  return <div className="shell">
    <aside><div className="brand"><img src={logo} alt="FileX Backup Guard"/><div><span>FILEX SUITE</span><h1>Backup Guard</h1><p>Protezione archivio</p></div></div><nav>{nav.map(([id, title, small], index) => <button className={screen === id ? "active" : ""} onClick={() => setScreen(id)} key={id}><span>{index + 1}</span><div><strong>{title}</strong><small>{small}</small></div></button>)}</nav><div className="suite-pill">FILEX SUITE <strong>Sincronizzazione verificata</strong></div></aside>
