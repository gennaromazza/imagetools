...
export function locateEmbeddedJpegRange(buffer) {
    const data = new Uint8Array(buffer);
    if (data.length < 12)
        return null;
    const candidates = [];
    const tiffCandidate = tryTiffLocate(data);
    if (tiffCandidate && tiffCandidate.length > 10_000)
        candidates.push(tiffCandidate);
    const rafCandidate = tryRafLocate(data);
    if (rafCandidate && rafCandidate.length > 10_000)
        candidates.push(rafCandidate);
    const bmffCandidate = tryBmffLocate(data);
    if (bmffCandidate && bmffCandidate.length > 10_000)
        candidates.push(bmffCandidate);
...
export function extractEmbeddedJpeg(buffer) {
    ...
    if (tiffResult && tiffResult.byteLength > 10_000)
        candidates.push(tiffResult);
    const cr3Result = tryBmffExtract(data, buffer);
    if (cr3Result && cr3Result.byteLength > 10_000)
        candidates.push(cr3Result);
    const rafResult = tryRafExtract(data, buffer);
    if (rafResult && rafResult.byteLength > 10_000)
        candidates.push(rafResult);
    const structured = scanForLargestJpeg(data, buffer);
    if (structured && structured.byteLength > 10_000)
    ...
    const normalized = candidates
        ...
        .filter((candidate) => candidate !== null && candidate.byteLength > 10_000);
...
function normalizeJpegCandidate(candidate) {
...
function getJpegPixelCount(candidate) {
...
function extractJpegSliceAtOffset(data, buffer, start, maxEndExclusive) {
    if (start < 0 ||
        start + 2 >= data.length ||
...
function pushCandidateRange(candidates, offset, length) {
    if (offset > 0 && length > 10_000) {
        candidates.push({ offset, length });
    }
}
function tryTiffLocate(data) {
    const b0 = data[0];
    const b1 = data[1];
    let le;
    if (b0 === 0x49 && b1 === 0x49) {
    ...
    if (firstIfdOffset === 0 || firstIfdOffset >= data.length)
        return null;
    const candidates = [];
...
function tryTiffExtract(data, buffer) {
    ...
    const magic = readU16(data, 2, le);
    if (magic !== 42 && magic !== 0x4f52)
        return null; // 42 = standard TIFF, 0x4f52 = ORF variant
    const firstIfdOffset = readU32(data, 4, le);
    if (firstIfdOffset === 0 || firstIfdOffset >= data.length)
        return null;
    ...
    while (ifdOffset > 0 &&
        ...
        if (result.jpegOffset > 0 && result.jpegLength > 0) {
            const candidate = extractJpegSliceAtOffset(data, buffer, result.jpegOffset, result.jpegOffset + result.jpegLength);
        ...
        if (result.stripOffset > 0 && result.stripLength > 0) {
            const candidate = extractJpegSliceAtOffset(data, buffer, result.stripOffset, result.stripOffset + result.stripLength);
            if (candidate && candidate.byteLength > bestSize) {
                bestJpeg = candidate;
                bestSize = candidate.byteLength;
            }
        ...
        for (const subOffset of result.subIfdOffsets) {
            if (subOffset > 0 && subOffset < data.length - 2) {
                ...
                if (sub.jpegOffset > 0 && sub.jpegLength > 0) {
                    const candidate = extractJpegSliceAtOffset(data, buffer, sub.jpegOffset, sub.jpegOffset + sub.jpegLength);
                    if (candidate && candidate.byteLength > bestSize) {
                        bestJpeg = candidate;
                        bestSize = candidate.byteLength;
                    }
                ...
                if (sub.stripOffset > 0 && sub.stripLength > 0) {
                    const candidate = extractJpegSliceAtOffset(data, buffer, sub.stripOffset, sub.stripOffset + sub.stripLength);
                    if (candidate && candidate.byteLength > bestSize) {
                        bestJpeg = candidate;
                        bestSize = candidate.byteLength;
                    }
...
function parseIfd(data, _buffer, offset, le) {
    const result = {
        jpegOffset: 0,
        jpegLength: 0,
        stripOffset: 0,
    ...
    for (let i = 0; i < entryCount; i++) {
        ...
        switch (tag) {
            ...
            case TAG_SUB_IFD: {
                ...
                if (count === 1) {
                    result.subIfdOffsets.push(readValueU32(data, valueOffset, type, count, le));
                }
...
function tryRafExtract(data, buffer) {
...
function tryBmffExtract(data, buffer) {
    if (data.length < 8)
        return null;
 ...
 */
function scanForJpegInRange(data, buffer, rangeStart, rangeEnd) {
    ...
    while (i < len - 3) {
        ...
        while (j < len - 1) {
            ...
            if (marker === 0xd8)
                continue; // extra SOI — skip
            if (marker === 0x00)
                continue; // byte stuffing
            if (marker >= 0xd0 && marker <= 0xd7)
                continue; // RST0-RST7, no length
            if (marker === 0xda) {
...
function scanForLargestJpeg(data, buffer) {
 ...
 */
function scanForLargestJpegByMarkers(data, buffer) {
    let bestStart = -1;
    let bestSize = 0;
    for (let i = 0; i < data.length - 1; i++) {
        if (data[i] !== JPEG_SOI_0 || data[i + 1] !== JPEG_SOI_1)
            continue;
...
// ── Binary helpers ─────────────────────────────────────────────────────
function readU16(data, offset, le) {
    if (offset + 2 > data.length)
        return 0;
    return le
...
function readU32(data, offset, le) {
...
function readValueU32(data, valueOffset, type, count, le) {
    if (count !== 1) {
    ...
    if (type === 3)
        return readU16(data, valueOffset, le);
    return readU32(data, valueOffset, le);
}
/**
 ...
 */
function isDecodableJpeg(data, offset, length) {
    if (length < 10)
        return false;
...
//# sourceMappingURL=raw-jpeg-extractor.js.map