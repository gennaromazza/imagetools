...
const RAM_PRESET_OPTIONS = [
    { preset: "conservative", label: "Conservativo", fraction: 0.06 },
    { preset: "default", label: "Default", fraction: 0.12 },
    { preset: "performance", label: "Performance", fraction: 0.20 },
    { preset: "maximum", label: "Massimo", fraction: 0.28 },
];
function RamBudgetSection({ systemTotalMemoryBytes, activePreset, activeRamBudgetBytes, onPresetChange, onRelaunch, }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsx("label", { className: "photo-selector__settings-color-row", style: { alignItems: "center", marginTop: "0.6rem" }, children: _jsx("span", { style: { fontSize: "0.7rem", color: "var(--text-muted)", minWidth: 90 }, children: "Budget RAM" }) }), _jsx("div", { className: "photo-selector__settings-preset-row", style: { flexWrap: "wrap", gap: "0.3rem" }, children: RAM_PRESET_OPTIONS.map(({ preset, label, fraction }) => {
                    const gb = ((systemTotalMemoryBytes * fraction) / (1024 ** 3)).toFixed(1);
                    const isSelected = displayPreset === preset;
                    const isActive = activePreset === preset;
                    return (_jsxs("button", { type: "button", className: `ghost-button ghost-button--small${isSelected ? " ghost-button--active" : ""}`, onClick: () => setPendingPreset(preset), title: `${label}: ${gb} GB (${Math.round(fraction * 100)}% RAM)${isActive ? " — preset corrente" : ""}`, style: isActive && !isSelected ? { opacity: 0.55 } : undefined, children: [label, " (", gb, " GB)"] }, preset));
                }) }), hasPendingChange ? (_jsxs("div", { style: { display: "flex", alignItems: "center", gap: "0.5rem", marginTop: "0.35rem", flexWrap: "wrap" }, children: [_jsx("button", { type: "button", className: "secondary-button", style: { fontSize: "0.8rem", padding: "0.4rem 0.8rem" }, onClick: () => void handleApply(), disabled: applying, children: applying ? "Salvo…" : "Applica e riavvia" }), _jsx("button", { type: "button", className: "ghost-button ghost-button--small", onClick: () => setPendingPreset(null), disabled: applying, children: "Annulla" })] })) : (_jsx("p", { className: "photo-selector__settings-empty", style: { marginTop: "0.3rem" }, children: activePreset
...
export function PhotoSelector({ photos, metadataVersion, sourceFolderPath = "", selectedIds, onSelectionChange, onPhotosChange, onVisibleIdsChange, onPriorityIdsChange, onPreviewPriorityIdsChange, onBackgroundPreviewOrderChange, onScrollLiteActiveMsChange, onUndo, onRedo, canUndo = false, canRedo = false, isThumbnailLoading = false, thumbnailProfile = "ultra-fast", sortCacheEnabled = true, performanceSnapshot = null, onThumbnailProfileChange, onSortCacheEnabledChange, desktopThumbnailCacheInfo = null, desktopCacheLocationRecommendation = null, isDesktopThumbnailCacheBusy = false, isDesktopCacheRecommendationModalOpen = false, onChooseDesktopThumbnailCacheDirectory, onSetDesktopThumbnailCacheDirectory, onUseRecommendedDesktopThumbnailCacheDirectory, onResetDesktopThumbnailCacheDirectory, onClearDesktopThumbnailCache, onSnoozeDesktopCacheRecommendation, onDismissDesktopCacheRecommendation, onRamBudgetPresetChange, onRelaunch, }) {
    ...
    const sortedPhotoIds = useMemo(() => {
        ...
        const orderedIds = metadataPhotos
            ...
            if (sortBy === "createdAt") {
                ...
                return ((createdAtSortDirection === "asc" ? createdAtDiff : -createdAtDiff) ||
                    left.fileName.localeCompare(right.fileName));
            }
    ...
    const viewportPriorityIds = useMemo(() => {
        ...
        if (virtualRows.length === 0) {
            const fallbackCount = Math.min(visiblePhotoIds.length, Math.max(gridColumnCount * (prefetchBefore + prefetchAfter + 2), gridColumnCount * 6));
    ...
    useEffect(() => {
        ...
        const ids = new Set(viewportPriorityIds);
