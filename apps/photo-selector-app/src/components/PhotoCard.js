...
export const PhotoCard = memo(function PhotoCard({ photo, isSelected, onToggle, onUpdatePhoto, onFocus, onPreview, onContextMenu, editable, }) {
    ...
    return (_jsxs("div", { className: `photo-card ${isSelected ? "photo-card--selected" : ""}`, role: "option", tabIndex: 0, "aria-selected": isSelected, "aria-label": `${photo.fileName}${isSelected ? ", selezionata" : ", non selezionata"}`, "aria-keyshortcuts": "Enter Space 1 2 3 4 5 P X U", "data-preview-asset-id": photo.id, onClick: (event) => onToggle(photo.id, event), onFocus: () => onFocus(photo.id), onMouseEnter: () => {
            if (photo.previewUrl)
...
}, (prev, next) => prev.photo === next.photo &&
