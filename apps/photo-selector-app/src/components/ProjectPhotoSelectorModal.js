...
export function ProjectPhotoSelectorModal({ assets, activeAssetIds, usageByAssetId, customLabelsCatalog = [], customLabelColors = {}, customLabelShortcuts = {}, onClose, onApply }) {
    ...
    const localAssetById = useMemo(() => new Map(localAssets.map((asset) => [asset.id, asset])), [localAssets]);
    const visibleAssetIndexById = useMemo(() => new Map(visibleAssets.map((asset, index) => [asset.id, index])), [visibleAssets]);
    const gridColumnCount = useMemo(() => {
        const width = gridViewport.width || MODAL_CARD_MIN_WIDTH_PX;
        return Math.max(1, Math.floor((width + MODAL_GRID_GAP_PX) / (MODAL_CARD_MIN_WIDTH_PX + MODAL_GRID_GAP_PX)));
    }, [gridViewport.width]);
    const gridColumnWidth = useMemo(() => {
        const width = gridViewport.width || MODAL_CARD_MIN_WIDTH_PX;
        return Math.max(MODAL_CARD_MIN_WIDTH_PX, Math.floor((width - MODAL_GRID_GAP_PX * Math.max(0, gridColumnCount - 1)) / gridColumnCount));
    }, [gridColumnCount, gridViewport.width]);
    const estimatedCardHeight = useMemo(() => Math.round(gridColumnWidth * 0.75) + MODAL_CARD_ESTIMATED_EXTRA_HEIGHT_PX, [gridColumnWidth]);
    const effectiveCardHeight = modalCardHeight > 0 ? modalCardHeight : estimatedCardHeight;
    const gridRowHeight = useMemo(() => Math.max(1, effectiveCardHeight + MODAL_GRID_GAP_PX), [effectiveCardHeight]);
    const totalVirtualRows = useMemo(() => Math.max(1, Math.ceil(visibleAssets.length / gridColumnCount)), [gridColumnCount, visibleAssets.length]);
    const rowVirtualizer = useVirtualizer({
        count: totalVirtualRows,
        getScrollElement: () => gridRef.current,
        estimateSize: () => gridRowHeight,
        overscan: isFastScrollActive ? MODAL_VIRTUAL_OVERSCAN_ROWS_FAST : MODAL_VIRTUAL_OVERSCAN_ROWS_IDLE
    });
    ...
    const renderedAssets = useMemo(() => {
        ...
        for (const row of virtualRows) {
            ...
            const rowEnd = Math.min(visibleAssets.length, rowStart + gridColumnCount);
        ...
        return rendered;
    }, [gridColumnCount, virtualRows, visibleAssets]);
    ...
    useEffect(() => {
        ...
        const frameId = window.requestAnimationFrame(() => {
            ...
            const measuredHeight = Math.round(firstCard.getBoundingClientRect().height);
            if (!Number.isFinite(measuredHeight) || measuredHeight <= 0) {
                return;
            }
            setModalCardHeight((current) => (current === measuredHeight ? current : measuredHeight));
        });
        return () => window.cancelAnimationFrame(frameId);
    }, [gridColumnWidth, renderedAssets]);
    useEffect(() => {
        const grid = gridRef.current;
        if (grid) {
            grid.scrollTo({ top: 0 });
        ...
        flushFastScrollMode();
    }, [flushFastScrollMode, rowVirtualizer, sortBy, pickFilter, usageFilter, colorFilter, ratingFilter]);
    useEffect(() => {
        if (!contextMenuState) {
            return;
        }
    ...
    useEffect(() => {
        const handleKeyDown = (event) => {
            ...
            if (currentIndex < 0) {
                ...
                nextIndex = Math.min(visibleAssets.length - 1, currentIndex + gridColumnCount);
            }
            else {
                // ArrowUp
                nextIndex = Math.max(0, currentIndex - gridColumnCount);
            }
            if (nextIndex !== currentIndex || currentIndex < 0) {
                ...
                const grid = gridRef.current;
                if (grid) {
                    const rowIndex = Math.floor(nextIndex / gridColumnCount);
                    ...
                    if (rowTop < viewportTop) {
                        ...
                        grid.scrollTo({
                            top: Math.max(0, rowBottom - grid.clientHeight),
                            behavior: "smooth"
                        });
                    }
                }
                window.requestAnimationFrame(() => {
                    const button = gridRef.current?.querySelector(`[data-preview-asset-id="${nextAsset.id}"]`);
                    button?.focus();
                });
            }
        };
    ...
    const modalContent = (_jsxs(_Fragment, { children: [_jsxs("div", { className: "modal-panel modal-panel--wide", onClick: (event) => event.stopPropagation(), children: [_jsxs("div", { className: "modal-panel__header", children: [_jsxs("div", { children: [_jsx("strong", { children: "Selezione foto del progetto" }), _jsx("p", { children: "Rivedi le foto a schermo grande, assegna stelle e colori e scegli quali entrano davvero nel layout." })] }), _jsxs("div", { className: "button-row", children: [_jsx(PhotoClassificationHelpButton, { title: "Scorciatoie selezione progetto" }), _jsx("button", { type: "button", className: "ghost-button", onClick: onClose, children: "Chiudi" })] })] }), _jsxs("div", { className: "modal-toolbar modal-toolbar--selector", children: [_jsxs("div", { className: "button-row", children: [_jsx("button", { type: "button", className: "ghost-button", onClick: () => setLocalSelection(hasActiveFilters
                                ...
                                return (_jsxs("button", { type: "button", "data-preview-asset-id": asset.id, className: [
                                        "modal-photo-card",
                                        isSelected ? "modal-photo-card--active" : "",
                                        isFastScrollActive ? "modal-photo-card--scroll-lite" : "",
                                    ].filter(Boolean).join(" "), onClick: () => toggleAsset(asset.id), onFocus: () => setFocusedAssetId(asset.id), onMouseEnter: () => {
                                        if (!isFastScrollActive && asset.previewUrl)
                            ...
                            }), bottomSpacerHeight > 0 ? (_jsx("div", { className: "modal-photo-grid__virtual-spacer", style: { height: bottomSpacerHeight }, "aria-hidden": "true" })) : null] }), _jsxs("div", { className: "modal-panel__footer", children: [_jsx("p", { className: "selector-shortcuts", children: "Usa Info per tutte le scorciatoie. Spazio apre la preview, 1-5 assegna stelle, P/X/U cambia stato e Ctrl/Cmd + 6/7/8/9/V assegna i colori." }), _jsxs("div", { className: "button-row", children: [_jsx("button", { type: "button", className: "ghost-button", onClick: onClose, children: "Annulla" }), _jsxs("button", { type: "button", className: "primary-button", onClick: () => onApply(localSelection, localAssets), children: ["Usa ", localSelection.length, " foto nel progetto"] })] })] })] }), _jsx(PhotoQuickPreviewModal, { asset: previewAsset, assets: visibleAssets, usageByAssetId: usageByAssetId, customLabelsCatalog: customLabelsCatalog, customLabelColors: customLabelColors, customLabelShortcuts: customLabelShortcuts, onClose: () => setPreviewAssetId(null), onSelectAsset: setPreviewAssetId, onUpdateAsset: updateAsset }), contextMenuState ? (_jsx(PhotoColorContextMenu, { x: contextMenuState.x, y: contextMenuState.y, selectedColor: localAssetById.get(contextMenuState.assetId)?.colorLabel ?? null, title: "Etichetta colore", onSelect: (colorLabel) => {
