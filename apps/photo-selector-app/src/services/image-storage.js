...
let dbPromise = null;
function getFileLookupKey(file) {
...
export async function saveImageAssets(projectId, files, imageAssets) {
    const fileMap = new Map(files.map((file) => [getFileLookupKey(file), file]));
    const preparedAssets = await Promise.all(imageAssets.map(async (asset) => {
        const sourceBlob = fileMap.get(asset.sourceFileKey ?? asset.path ?? asset.fileName) ??
            (await blobFromUrl(asset.sourceUrl ?? asset.previewUrl ?? asset.thumbnailUrl));
        if (!sourceBlob) {
            return null;
        }
        const thumbnailBlob = asset.thumbnailUrl && asset.thumbnailUrl !== asset.sourceUrl
            ? await blobFromUrl(asset.thumbnailUrl)
            : undefined;
        const previewBlob = asset.previewUrl &&
