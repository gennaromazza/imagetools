...
function touchQuickPreviewFrameCacheEntry(
  cacheKey: string,
...
function trimQuickPreviewFrameCache(): void {
  while (quickPreviewFrameCache.size > QUICK_PREVIEW_FRAME_CACHE_MAX_ENTRIES) {
    const oldest = quickPreviewFrameCache.entries().next().value as [string, DesktopQuickPreviewFrame] | undefined;
    if (!oldest) {
      break;
    }
...
export function hasDesktopQuickPreviewApi(): boolean {
  const api = getDesktopApi();
  return Boolean(
    typeof api?.getQuickPreviewFrame === "function"
...
export function getCachedDesktopQuickPreviewFrame(
  request: DesktopQuickPreviewRequest,
): DesktopQuickPreviewFrame | null {
...
export function peekDesktopQuickPreviewFrame(
  request: DesktopQuickPreviewRequest,
): DesktopQuickPreviewFrame | null {
...
): Promise<DesktopQuickPreviewWarmResult | null> {
  const api = getDesktopApi();
...
export async function releaseDesktopQuickPreviewFrames(tokens: string[]): Promise<void> {
  const api = getDesktopApi();
