...
import { buildPlaceholderAssets, addRecentFolder, hasNativeFolderAccess, isRawFile, readSidecarXmp, writeSidecarXmp, } from "./services/folder-access";
...
export function App() {
    ...
    const handleThumbnailError = useCallback((failedCount) => {
        const pipeline = pipelineRef.current;
        if (pipeline) {
            const settledCount = pipeline.completedCount + pipeline.failedCount;
            setThumbnailProgress({ done: settledCount, total: settledCount + pipeline.pendingCount });
    ...
    const handleFolderOpened = useCallback((folderName, entries) => {
        if (entries.length === 0) {
        ...
        void loadCachedThumbnails(allIds).then((cached) => {
            ...
            if (cached.size > 0) {
                setAllAssets((prev) => prev.map((asset) => {
                    ...
                    if (!hit)
                        return asset;
                    if (!isValidCachedThumbnail(asset, hit))
                        return asset;
