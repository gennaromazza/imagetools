...
import { chooseDesktopThumbnailCacheDirectory, clearDesktopThumbnailCache, getDesktopThumbnailCacheInfo, resetDesktopThumbnailCacheDirectory, setDesktopThumbnailCacheDirectory, } from "./services/desktop-thumbnail-cache";
import { loadImageAssets } from "./services/image-storage";
import { clearImageCache } from "./services/image-cache";
import { buildPlaceholderAssets, addRecentFolder, buildSourceFileKey, buildSourceFileKeyFromStats, getFileForAsset, hasNativeFolderAccess, isRawFile, readSidecarXmp, writeSidecarXmp, } from "./services/folder-access";
...
import { beginReactBatchMetric, cancelReactBatchMetric, finishReactBatchMetric, perfTime, perfTimeEnd, resetPerfByteReadStats, } from "./services/performance-utils";
...
export function App() {
    ...
    const undoRedo = useUndoRedo(() => allAssetsRef.current, (snapshot) => setAllAssets(snapshot));
    ...
    function isValidCachedThumbnail(asset, hit) {
    ...
    const handleFolderOpened = useCallback(({ name: folderName, entries, rootPath, diagnostics }) => {
    ...
    useEffect(() => {
        ...
        xmpSyncTimerRef.current = window.setTimeout(() => {
            ...
            void Promise.all(idsToSync.map(async (assetId) => {
