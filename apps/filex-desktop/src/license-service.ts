...
export async function getLicenseState(refresh = false): Promise<DesktopLicenseState> {
  ...
  const token = await decryptToken(store.activationTokenEncrypted);
  if (token && process.platform === "win32" && store.activationTokenEncrypted && !store.activationTokenEncrypted.startsWith(SHARED_TOKEN_PREFIX)) {
